...
beforeAll(async () => {
  ...
  sender = new MessageSender(config.planEventQueueConfig, queueUrl)
})
...
describe('consume message', () => {
  ...
  test('consume a json message', (done) => {
    sender.sendMessage({ greeting: greeting }).then(() => {
      consumer = new MessageConsumer(config.planEventQueueConfig, queueUrl, (message) => messageHandler(message, done))
      consumer.start()
    })
  })
  test('message is redelivered after a client error', (done) => {
    ...
    sender.sendMessage({ greeting: redeliverGreeting }).then(() => {
      consumer = new MessageConsumer(config.planEventQueueConfig, queueUrl, (message) => messageHandlerErrorOnFirst(message, done))
      consumer.start()
    })
  })
})
